...
## read data
###############################################################################################
APSIM_PT_SHT_S2 <- get(load("D:/dev_agwise/AgWISE-UseCaseRAB/02_plantingWLY/APSIM/OutputData/APSIM_PT_SHT_S2.Rdata") )
APSIM_PT_MED_S2 <- get(load("D:/dev_agwise/AgWISE-UseCaseRAB/02_plantingWLY/APSIM/OutputData/APSIM_PT_MED_S2.Rdata") )
APSIM_PT_LNG_S2 <- get(load("D:/dev_agwise/AgWISE-UseCaseRAB/02_plantingWLY/APSIM/OutputData/APSIM_PT_LNG_S2.Rdata") )
...
## add sowing year
APSIM_PT_SHT_S2$SowYear <- year(ymd_hms(APSIM_PT_SHT_S2$Potato.SowingDate))
APSIM_PT_MED_S2$SowYear <- year(ymd_hms(APSIM_PT_MED_S2$Potato.SowingDate))
APSIM_PT_LNG_S2$SowYear <- year(ymd_hms(APSIM_PT_LNG_S2$Potato.SowingDate))


## indicate season incase we want to combine the two seasons data
APSIM_PT_SHT_S2$season <- "S2"
APSIM_PT_MED_S2$season <- "S2"
APSIM_PT_LNG_S2$season <- "S2"


## get sowing dates to order the leevls in ggplot
unique(APSIM_PT_SHT_S2$SowDate)
unique(APSIM_PT_MED_S2$SowDate)
unique(APSIM_PT_LNG_S2$SowDate)
...
## Potato simulation does not have location, get it from 
...
APSIM_PT_SHT_S2 <- merge(APSIM_PT_SHT_S2, RAB_Location[, c("lonlat", "Location")], by="lonlat")
APSIM_PT_MED_S2 <- merge(APSIM_PT_MED_S2, RAB_Location[, c("lonlat", "Location")], by="lonlat")
APSIM_PT_LNG_S2 <- merge(APSIM_PT_LNG_S2, RAB_Location[, c("lonlat", "Location")], by="lonlat")
...
mapData <- APSIM_PT_MED_S2[APSIM_PT_MED_S2$SowDate == "15-jul" & APSIM_PT_MED_S2$SowYear == 2005, ]
ggplot() +
...
APSIM_PT_SHT_S2$index <- c(1:nrow(APSIM_PT_SHT_S2))
APSIM_PT_MED_S2$index <- c(1:nrow(APSIM_PT_MED_S2))
APSIM_PT_LNG_S2$index <- c(1:nrow(APSIM_PT_LNG_S2))

W_data_S <- APSIM_PT_SHT_S2[APSIM_PT_SHT_S2$Longitude < 29.2 & 
                              APSIM_PT_SHT_S2$Latitude < -1.8 & APSIM_PT_SHT_S2$Latitude > -2.4, ]
W_data_M <- APSIM_PT_MED_S2[APSIM_PT_MED_S2$Longitude < 29.2 & 
                              APSIM_PT_MED_S2$Latitude < -1.8 & APSIM_PT_MED_S2$Latitude > -2.4, ]
W_data_L <- APSIM_PT_LNG_S2[APSIM_PT_LNG_S2$Longitude < 29.2 & 
                              APSIM_PT_LNG_S2$Latitude < -1.8 & APSIM_PT_LNG_S2$Latitude > -2.4, ]

APSIM_PT_SHT_S2 <- droplevels(APSIM_PT_SHT_S2[!APSIM_PT_SHT_S2$index %in% W_data_S$index, ])
APSIM_PT_MED_S2 <- droplevels(APSIM_PT_MED_S2[!APSIM_PT_MED_S2$index %in% W_data_M$index, ])
APSIM_PT_LNG_S2 <- droplevels(APSIM_PT_LNG_S2[!APSIM_PT_LNG_S2$index %in% W_data_L$index, ])

### creating the season 2 data across the three cultivars

APSIM_PT_SHT_S2$cultivar <- "Short"
APSIM_PT_MED_S2$cultivar <- "Medium"
APSIM_PT_LNG_S2$cultivar <- "Long"

APSIM_PT_S2 <- rbind(APSIM_PT_SHT_S2, APSIM_PT_MED_S2, APSIM_PT_LNG_S2)

# APSIM_PT_S2<-subset(APSIM_PT_S2, SowDate  %in% c("01-jul","08-jul", "15-jul", "29-jul",
#                                                  "05-aug", "12-aug", "19-aug", "26-aug",
#                                                  "02-sep", "09-sep" ,"16-sep", "23-sep", "30-sep",
#                                                  "07-oct", "14-oct", "21-oct", "28-oct"))
unique(APSIM_PT_S2$SowDate)
###############################################################################################
## plotting for general observations
###############################################################################################
unique(APSIM_PT_S2$Location)
## across locations and years: very similar mean yield across sowing dates but different in variation 

ggplot(APSIM_PT_S2, aes(x= factor(SowDate, levels = S2_SowDate), y = Yield, col=factor(cultivar))) +
  geom_boxplot() +
...
ggplot(APSIM_PT_S2, aes(x= factor(SowYear), y = Yield, col=factor(cultivar)))+
  geom_boxplot() +
  facet_wrap(~Location) +
...
ggplot(APSIM_PT_S2, aes(x= factor(SowYear), y = InCropRainfall, col=factor(cultivar)))+
  geom_boxplot() +
  facet_wrap(~Location) +
...
ggplot(APSIM_PT_S2, aes(x= factor(SowYear), y = TotalESW, col=factor(cultivar)))+
  geom_boxplot() +
  facet_wrap(~Location) +
...
length(unique(APSIM_PT_S2[APSIM_PT_S2$Location == "Iburengerazuba", "lonlat"]))##131
length(unique(APSIM_PT_S2[APSIM_PT_S2$Location == "Amajyepfo", "lonlat"]))##197
length(unique(APSIM_PT_S2[APSIM_PT_S2$Location == "Amajyaruguru", "lonlat"]))##101
length(unique(APSIM_PT_S2[APSIM_PT_S2$Location == "Iburasirazuba", "lonlat"]))##288
length(unique(APSIM_PT_S2[APSIM_PT_S2$Location == "Umujyi wa Kigali","lonlat"]))##21
...
locations <- unique(APSIM_PT_S2$Location)
loc1 <- APSIM_PT_S2[APSIM_PT_S2$Location == locations[1], ]
...
Summaries <- ddply(APSIM_PT_S2, .(Longitude, Latitude, lonlat, SowDate, cultivar), summarize,
...
##########################JANE###########################################################################
ggplot() +
  geom_point(data = APSIM_PT_LNG_S2, aes(WUE2, Yield, color = SowDate, group = SowYear))+
  facet_wrap(~Location) +
  scale_shape_manual()+
  theme_bw()+
...
ggplot() +
  geom_point(data = APSIM_PT_LNG_S2, aes(InCropRainfall, Yield, color = SowDate, group = SowYear))+
  facet_wrap(~Location) +
  scale_shape_manual()+
  theme_bw()+
  theme(legend.position = "left")
...
ggplot() +
  geom_point(data = APSIM_PT_LNG_S2, aes(InCropRainfall, InCropMaxTemperature, color = SowDate, group = SowYear))+
  facet_wrap(~Location) +
  scale_shape_manual()+
  theme_bw()+
  theme(legend.position = "left")
